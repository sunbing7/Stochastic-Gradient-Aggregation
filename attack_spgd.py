...
def uap_spgd_targeted(model, loader, nb_epoch, eps, beta=9, step_decay=0.1, loss_function=None,
                      targeted=0, target_class=0,
    ...
    for epoch in range(nb_epoch):
        ...
        for i, data in enumerate(loader):
            ...
            if loss_function:
                if targeted:
                    loss = -clamped_loss(outputs, target_class_label.cuda())
                else:
                    loss = clamped_loss(outputs, target_label.cuda())
            else:
                if targeted:
                    loss = torch.mean(outputs.gather(1, (target_class_label.cuda()).unsqueeze(1)).squeeze(1))
                else:
                    loss = -torch.mean(outputs.gather(1, (target_label.cuda()).unsqueeze(1)).squeeze(1))
