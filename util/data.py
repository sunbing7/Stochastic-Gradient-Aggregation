...
def get_data(dataset, pretrained_dataset, preprocess=None, is_attack=False):
    ...
    if dataset == "imagenet":
        ...
        train_data_full = torch.utils.data.Subset(full_val, index_train)
        test_data_full = torch.utils.data.Subset(full_val, index_test)
        test_data = torch.utils.data.Subset(test_data_full,
                                            np.random.choice(len(test_data_full),
                                            size=1000,
                                            replace=False))
        ...
        if is_attack:
            train_data = torch.utils.data.Subset(train_data_full,
                                                 np.random.choice(len(train_data_full),
                                                                   size=min(int(0.5 * len(train_data_full)), 10000),
                                                                   replace=False))
        else:
            train_data = torch.utils.data.Subset(train_data_full, np.random.choice(len(train_data_full),
                                                 size=min(int(0.05 * len(train_data_full)), 10000), replace=False))
        ...
        if is_attack:
            train_data = torch.utils.data.Subset(train_data_full, np.random.choice(len(train_data_full),
                                                 size=min(int(0.5 * len(train_data_full)), 10000), replace=False))
        else:
            train_data = torch.utils.data.Subset(train_data_full, np.random.choice(len(train_data_full),
                                                 size=min(int(0.05 * len(train_data_full)), 10000), replace=False))
        test_data_full = dset.ImageFolder(root=testdir, transform=test_transform)
        test_data = torch.utils.data.Subset(test_data_full,
                                            np.random.choice(len(test_data_full),
                                            size=1000,
                                            replace=False))
        ...
        if is_attack:
            ...
            train_data = torch.utils.data.Subset(train_data_full,
                                                 np.random.choice(len(train_data_full),
                                                                  size=min(int(0.05 * len(train_data_full)), 10000),
                                                                  replace=False))
        _, test_data_full = random_split(test_dataset, 0.9, random_state=42)
        test_data = torch.utils.data.Subset(test_data_full,
                                            np.random.choice(len(test_data_full),
                                            size=1000,
                                            replace=False))
        ...
        if is_attack:
            ...
            train_data = torch.utils.data.Subset(train_data_full,
                                                 np.random.choice(len(train_data_full),
                                                                  size=min(int(0.05 * len(train_data_full)), 10000),
                                                                  replace=False))
        _, test_data_full = random_split(testset, 0.9, random_state=42)
